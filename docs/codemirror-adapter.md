...
Import: `@marimo-team/codemirror-sql/codemirror`

`sqlEditor` connects one caller-owned SQL language service to one or more
...
import {
  sqlEditor,
} from "@marimo-team/codemirror-sql/codemirror";
...
The adapter installs one coherent `autocompletion` configuration and registers
its source through CodeMirror language data. Existing language sources, such as
SQL keywords and functions from `@codemirror/lang-sql`, remain active.
Additional global sources belong in `autocomplete.externalSources`; consumers
should not install a second independently configured `autocompletion`
extension.
...
The adapter deliberately does not use `validFor` or result mapping because
schema and scope changes can invalidate options without changing the typed
prefix. Every result and completion application rechecks the session revision,
